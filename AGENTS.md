...
Review submissions use Browser Subagent to open `https://chatgpt.com/` and type the concise prompt into the prompt textarea (using Shift+Enter for newlines, then Enter to send). Review begins with a fresh ChatGPT conversation on round 1 and reuses the same conversation URL across rounds.
...
After fixing findings locally, verify again, push the new HEAD, and run Browser Subagent review in the same conversation. Automatic merge remains OFF unless the user explicitly enables it.
