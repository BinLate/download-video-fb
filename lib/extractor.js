...
(function (root, factory) {
  ...
  function extractStreamsFromText(text) {
    ...
    if (dashParsed && !dashParsed.audioUrl) {
      const audioMatch = text.match(/"(?:audio_stream_url|audio_url|playable_url_quality_audio)"\s*:\s*"([^"]+)"/);
      if (audioMatch) {
        const directAudio = cleanMediaUrl(audioMatch[1]);
        if (directAudio && isValidMediaStream(directAudio)) {
          dashParsed.audioUrl = directAudio;
          dashParsed.isDashSeparate = true;
        }
      }
    }
  ...
  function extractUrlsFromScriptText(raw) {
    ...
    for (const text of texts) {
      const stack = []; // stores { start: number, hasStreamKey: boolean }
      let inString = false;
      let escaped = false;
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (!inString) {
          if (char === "{") {
            stack.push({ start: i, hasStreamKey: false });
          } else if (char === "}") {
            if (stack.length > 0) {
              const frame = stack.pop();
              const block = text.substring(frame.start, i + 1);

              // Check if this closed block contains stream keys
              const blockHasStream =
                frame.hasStreamKey ||
                block.includes("dash_manifest") ||
                block.includes("playback_video_dash_xml") ||
                block.includes("video_dash_manifest") ||
                block.includes("representations") ||
                block.includes("playable_url_quality_hd") ||
                block.includes("browser_native_hd_url") ||
                block.includes("playable_url") ||
                block.includes("browser_native_sd_url") ||
                block.includes("playable_url_dash") ||
                block.includes("dash_prefetch_experimental");

              if (blockHasStream) {
                if (stack.length > 0) {
                  stack[stack.length - 1].hasStreamKey = true;
                }

                // Prioritize video-specific ID patterns over generic unrelated IDs
                let videoId = null;
                const directVideoIdMatch = block.match(/"(?:video_id|videoId)"\s*:\s*"?(\d{6,30})"?/);
                if (directVideoIdMatch) {
                  videoId = directVideoIdMatch[1];
                } else {
                  const typenameVideoMatch = block.match(/"(?:__typename|type)"\s*:\s*"Video"[\s\S]{0,200}?"id"\s*:\s*"?(\d{6,30})"?/);
                  if (typenameVideoMatch) {
                    videoId = typenameVideoMatch[1];
                  } else {
                    const nodeVideoIdMatch = block.match(/"(?:video|node)"\s*:\s*\{[^{}]*?"id"\s*:\s*"?(\d{6,30})"?/);
                    if (nodeVideoIdMatch) {
                      videoId = nodeVideoIdMatch[1];
                    } else {
                      const genericIdMatch = block.match(/"id"\s*:\s*"?(\d{6,30})"?/);
                      if (genericIdMatch) {
