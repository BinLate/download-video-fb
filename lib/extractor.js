          ...
          if (char === "{") {
            ...
            if (stack.length > 0) {
              ...
              if (frame.hasStreamKey) {
                // An effective video ID must be:
                // 1. Explicit videoId ("video_id" or "videoId")
                // 2. An ID on an object explicitly identified as a video node on its own structural merits
                const effectiveId =
                  frame.videoId ||
                ...
                if (effectiveId) {
                  ...
                  // DO NOT propagate isVideoNode from child to parent! Parent must qualify as a video node on its own structural evidence!
