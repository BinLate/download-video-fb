...
(function (root, factory) {
  ...
  function extractStreamsFromText(text) {
    ...
    const audioMatch = text.match(/"(?:audio_stream_url|audio_url|audio_src|playable_url_quality_audio|browser_native_audio_url|audio_playback_url|playable_audio_url|audio_sd_src|audio_hd_src)"\s*:\s*"([^"]+)"/);
          ...
          if (!inString) {
            ...
            if (currentFrame) {
              if (
                ...
                strVal === "playable_url_quality_audio" ||
                strVal === "audio_playback_url" ||
                strVal === "playable_audio_url" ||
                strVal === "audio_sd_src" ||
                strVal === "audio_hd_src"
              ) {
              ...
              if (
                ...
                lastKey === "audio_playback_url" ||
                lastKey === "playable_audio_url" ||
                lastKey === "audio_sd_src" ||
                lastKey === "audio_hd_src"
              ) {
        ...
        if (!inString) {
          if (char === "{") {
            ...
            if (stack.length > 0) {
              ...
              // Order-independent: adopt pending child streams if parent is an explicit Video typename
              if (
                frame.pendingChildStreams &&
                frame.isExplicitVideoTypename &&
                !frame.directStreams.dashManifest &&
                !frame.directStreams.hdUrl &&
                !frame.directStreams.sdUrl
              ) {
                frame.directStreams = frame.pendingChildStreams;
                frame.hasDirectStreamKey = true;
              }
              ...
              if (frame.hasDirectStreamKey) {
                ...
                if (effectiveId) {
                  ...
                  // Stash pending streams on parent; parent will adopt only if completed parent is an explicit Video
                  const parent = stack[stack.length - 1];
                  parent.pendingChildStreams = frame.directStreams;
                }
              }
            }
