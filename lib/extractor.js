  ...
  function extractUrlsFromScriptText(raw) {
    ...
    for (const text of texts) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (!inString) {
          if (char === "{") {
            const isVideoProp = lastKey === "video" || lastKey === "story_video" || lastKey === "node";
            stack.push({
              start: i,
              hasStreamKey: false,
              videoId: null,
              candidateId: null,
              isVideoNode: isVideoProp
            });
            lastKey = null;
          } else if (char === "}") {
            if (stack.length > 0) {
              ...
              if (frame.hasStreamKey) {
                // An effective video ID is:
                // 1. Explicit videoId ("video_id" or "videoId")
                // 2. An ID on an object explicitly identified as a video node
                // 3. A top-level script object's ID when no ancestors exist
                const effectiveId =
                  frame.videoId ||
                  (frame.isVideoNode ? frame.candidateId : null) ||
                  (stack.length === 0 ? frame.candidateId : null);
                ...
                if (effectiveId) {
                  ...
                  // Association succeeded -> strictly DO NOT propagate upward to unrelated ancestors!
                } else if (stack.length > 0) {
                  // Propagate stream presence ONLY to the immediate parent so the parent video object can pair with it
                  stack[stack.length - 1].hasStreamKey = true;
                  if (frame.isVideoNode && !stack[stack.length - 1].isVideoNode) {
                    stack[stack.length - 1].isVideoNode = true;
                  }
                }
              }
            }
