    ...
    for (const text of texts) {
      for (const kw of streamKeywords) {
        ...
        while ((kwIdx = text.indexOf(`"${kw}"`, kwIdx)) !== -1) {
          const scanStart = Math.max(0, kwIdx - 4000);
          ...
          // Scan outward from closest opening brace to enclosing outer braces
          for (let b = openBraces.length - 1; b >= 0; b--) {
            ...
            let escaped = false;
            let endPos = -1;
            const maxSearch = Math.min(text.length, startPos + 25000);
            ...
            if (endPos > kwIdx) {
              ...
              if (videoId) {
                ...
                if (streams && (streams.hdUrl || streams.sdUrl || streams.audioUrl)) {
                  ...
                  // Only break when a valid videoId and streams pair was successfully identified
                  break;
                }
              }
              // If this candidate block did not yield an ID/stream, continue outward to the next enclosing brace!
            }
          }
