...
(function (root, factory) {
  ...
  function isDedicatedSingleVideoPage(pathname, search) {
    ...
    if (/^\/watch\/?$/i.test(path) && /[?&]v=\d{6,30}/i.test(searchStr)) return true;
    return false;
  }
          ...
          if (char === "{") {
            ...
            if (stack.length > 0) {
              ...
              if (frame.hasStreamKey) {
                ...
                } else if (stack.length > 0 && stack[stack.length - 1].isVideoNode) {
                  // Propagate stream presence ONLY if the immediate parent is a recognized video container
