...
(function (root, factory) {
      ...
      try {
        ...
        if (store.pending[token]) {
          ...
          store.pending[token] = typeof entry === "object"
            ? { ...entry, downloadId: String(downloadId) }
            : { url: entry, downloadId: String(downloadId) };
          await setSessionStorage(store, storageApi);
        }
      } catch (_) {}
    });
   ...
   * Reconciles both active map and pending fallback records to prevent orphan leaks.
   */
  async function unregisterBlobDownload(downloadId, storageApi = null) {
    ...
    return withQueue(async () => {
      ...
      let blobUrl = store.active[key] || null;
      if (blobUrl) {
        ...
        for (const [token, entry] of Object.entries(store.pending || {})) {
          ...
          if (entryDlId && String(entryDlId) === key) {
            blobUrl = entryUrl;
            delete store.pending[token];
            break;
          }
