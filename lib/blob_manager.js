 ...
 * Features dual-layer persistence: chrome.storage.session + in-memory fallback map + chrome.downloads reconciliation for zero-leak guarantees across worker restarts.
 */
...
(function (root, factory) {
  ...
  /**
   * Search Chrome downloads API for matching download item (used for restart reconciliation).
   */
  async function searchDownloadItem(downloadId, downloadsApi = null) {
    const api = downloadsApi || (typeof chrome !== "undefined" && chrome.downloads);
    if (api && typeof api.search === "function") {
      return new Promise((resolve) => {
        try {
          api.search({ id: Number(downloadId) }, (results) => {
            if (typeof chrome !== "undefined" && chrome.runtime?.lastError) {
              resolve(null);
            } else {
   ...
   * Fails fast and propagates error if durable session storage cannot be written.
   */
   ...
   * Reconciles:
   * 1. session storage active map
   * 2. session storage pending map by downloadId
   * 3. in-memory fallback map
   * 4. Chrome downloads API item URL reconciliation against pending entries (for SW restart recovery).
   */
  async function unregisterBlobDownload(downloadId, storageApi = null, downloadsApi = null) {
    ...
    return withQueue(async () => {
      ...
      try {
        ...
        if (blobUrl) {
          ...
          // 2. Reconcile with pending records matching downloadId
          ...
          // 3. SW restart recovery: if not matched, search Chrome download manager to correlate pending Blob URL
          if (!blobUrl && Object.keys(store.pending || {}).length > 0) {
            const downloadItem = await searchDownloadItem(downloadId, downloadsApi);
            if (downloadItem && downloadItem.url) {
              for (const [token, entry] of Object.entries(store.pending || {})) {
                const entryUrl = typeof entry === "string" ? entry : entry?.url;
                if (entryUrl === downloadItem.url) {
                  blobUrl = entryUrl;
                  delete store.pending[token];
                  break;
                }
        ...
        console.warn("[Bin.Late FB Downloader] Storage error during unregisterBlobDownload, checking fallback:", err.message);
      }
  ...
  async function revokeBlobUrl(blobUrl, { messageSender = null, offscreenCloser = null, storageApi = null } = {}) {
    ...
    if (typeof messageSender === "function") {
      ...
      try {
        chrome.runtime.sendMessage({ action: "OFFSCREEN_REVOKE_URL", blobUrl }, () => {
