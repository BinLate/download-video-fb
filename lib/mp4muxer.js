...
(function (root, factory) {
  ...
  function mergeFragmentedMp4Buffers(videoBuffer, audioBuffer) {
    ...
    // --- extract timescale & decode time for chronological fragment interleaving ---
    function getTrakTimescale(buffer, trakBox) {
      if (!trakBox) return 1000;
      const mdia = findBoxByType(buffer, "mdia", trakBox.start + trakBox.headerSize, trakBox.end);
      if (!mdia) return 1000;
      const mdhd = findBoxByType(buffer, "mdhd", mdia.start + mdia.headerSize, mdia.end);
      if (!mdhd) return 1000;
      const view = new DataView(buffer);
      const version = view.getUint8(mdhd.start + 8);
      const offset = version === 1 ? 20 : 12;
      return view.getUint32(mdhd.start + offset, false) || 1000;
    }

    function readFragmentDecodeTime(buffer, frag) {
      const trafList = findBoxes(buffer, frag.moof.start + frag.moof.headerSize, frag.moof.end).filter((b) => b.type === "traf");
      for (const traf of trafList) {
        const tfdt = findBoxByType(buffer, "tfdt", traf.start + traf.headerSize, traf.end);
        if (tfdt) {
          const view = new DataView(buffer);
          const version = view.getUint8(tfdt.start + 8);
          if (version === 1) {
            const high = view.getUint32(tfdt.start + 12, false);
            const low = view.getUint32(tfdt.start + 16, false);
            return high * 4294967296 + low;
          }
          return view.getUint32(tfdt.start + 12, false);
        }
      }
      return null;
    }

    const videoTraksList = collectTraks(videoBuffer, videoMoov);
    const videoTimescale = videoTraksList.length > 0 ? getTrakTimescale(videoBuffer, videoTraksList[0]) : 1000;
    const audioTimescale = audioTraks.length > 0 ? getTrakTimescale(audioBuffer, audioTraks[0]) : 1000;

    const rawPlan = [];
    videoFragments.forEach((frag, idx) => {
      const dt = readFragmentDecodeTime(videoBuffer, frag);
      const t = dt !== null ? dt / videoTimescale : idx * 1.0;
      rawPlan.push({ source: videoBuffer, frag, trackIdMap: null, time: t, isVideo: true });
    });
    audioFragments.forEach((frag, idx) => {
      const dt = readFragmentDecodeTime(audioBuffer, frag);
      const t = dt !== null ? dt / audioTimescale : idx * 1.0;
      rawPlan.push({ source: audioBuffer, frag, trackIdMap: audioTrackIdMap, time: t, isVideo: false });
    });

    // Interleave video and audio fragments chronologically so players decode audio & video concurrently
    rawPlan.sort((a, b) => (a.time - b.time) || (a.isVideo ? -1 : 1));
    ...
    for (const item of rawPlan) {
      ...
      cursor += item.frag.moof.size + (item.frag.mdat ? item.frag.mdat.size : 0);
    }
